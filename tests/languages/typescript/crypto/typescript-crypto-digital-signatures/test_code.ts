...
function basicSignatures() {
  ...
  const sign1: crypto.Sign = crypto.createSign('RSA-SHA256');
  sign1.update(data);
  ...
  const sign3: crypto.Sign = crypto.createSign('ecdsa-with-SHA256');
  sign3.update(data);
  const signature3 = sign3.sign(privateKey);

  const sign4: crypto.Sign = crypto.createSign('ecdsa-with-SHA512');
  sign4.update(data);
  const signature4 = sign4.sign(privateKey);

  const verify1: crypto.Verify = crypto.createVerify('RSA-SHA256');
  verify1.update(data);
  ...
  const verify3: crypto.Verify = crypto.createVerify('ecdsa-with-SHA256');
  verify3.update(data);
  const isValid3 = verify3.verify(publicKey, signature3);

  const verify4: crypto.Verify = crypto.createVerify('ecdsa-with-SHA512');
  verify4.update(data);
